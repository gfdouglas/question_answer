...
def index():
    ...
    db = get_db()
    questions_cur = db.execute('select q.id as question_id, q.question_text, ask.name as asker_name, exp.name  as exp_name \
                                from questions q \
                                join users ask on ask.id = q.asked_by_id \
                                join users exp on exp.id = q.expert_id \
...
def question(question_id):
    user = get_current_user()
    db = get_db()
    question_cur = db.execute('''select  q.question_text, q.answer_text, ask.name as asker_name, exp.name  as exp_name 
                                 from questions q 
                                 join users ask on ask.id = q.asked_by_id 
                                 join users exp on exp.id = q.expert_id 
